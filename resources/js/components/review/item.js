...
export default function Review() {
    ...
    this.init = function (review) {

        self.review = review;

        self.buttons = self.review.find('.buttons').first();

        self.descendants = self.review.find('.descendants').first();
        ...
        self.toggle_children.unbind('click').on('click', self.toggleDescendants);
    ...
    this.isChildsOpened = function () {
        return (self.descendants.find('.comment').length > 0);
    };

    this.toggleDescendants = function (event) {

        event.preventDefault();

        if (self.isChildsOpened()) {
            self.hideChildren();
        } else {
            self.openChildren();
        }
